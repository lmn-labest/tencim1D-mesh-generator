...
def test_generate(
    ...
    for node, (excepted, x) in enumerate(zip(expected_coor, mesh_with_standoff.x, strict=False)):
        assert excepted == pytest.approx(x, rel=1e-5), f'Node {node + 1}'

    for element, (excepted_, coon) in enumerate(zip(CONNECTIVITY, mesh_with_standoff.conn, strict=False)):
